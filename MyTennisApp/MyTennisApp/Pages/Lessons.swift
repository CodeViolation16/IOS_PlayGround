...
struct Lessons: View {
	 ...
	 @State private var list_date: [Date] = []
			...
			//	 var groupedLessons: [Date: [LessonModel]] {
			//			Dictionary(grouping: lessons, by: { Calendar.current.startOfDay(for: $0.date) })
			//	 }
			...
			VStack(alignment: .leading, spacing: 15) {
				 ...
				 HStack {
						Text(name)
							 .font(.title3)
							 .fontWeight(.bold)
							 .lineLimit(1) // Ensure the name doesn't wrap to a new line
							 .layoutPriority(1)
...
#Preview {
	 ...
	 // you submit date and more
	 // if date is same as previous date => add the list into the same date category
	 // if date is differ, render a new category
